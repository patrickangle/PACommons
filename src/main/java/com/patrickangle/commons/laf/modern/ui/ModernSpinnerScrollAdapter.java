...
public class ModernSpinnerScrollAdapter implements MouseWheelListener {
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
        ...
        if (model instanceof SpinnerNumberModel) {
            ...
            // The comparables will be numbers of some kind, but we need to maintain the type, otherwise some listeners will not work.
            Object originalNumber = numberModel.getValue();
            newValue = MathUtils.clamp(((Number) numberModel.getMinimum()).doubleValue(), newValue, ((Number) numberModel.getMaximum()).doubleValue());

            if (originalNumber instanceof Double || Double.TYPE.isInstance(originalNumber)) {
                numberModel.setValue((double) newValue);
            } else if (originalNumber instanceof Float || Float.TYPE.isInstance(originalNumber)) {
                numberModel.setValue((float) newValue);
            } else if (originalNumber instanceof Long || Long.TYPE.isInstance(originalNumber)) {
                numberModel.setValue((long) newValue);
            } else if (originalNumber instanceof Integer || Integer.TYPE.isInstance(originalNumber)) {
                numberModel.setValue((int) newValue);
            }
