...
/**
 * TalbeRowUtilities. Utility for adding a row column to a JTable http://blue-walrus.com/2014/12/row-number-column-in-jtable/
 *
...
public class RowHeaderUtils {
    ...
    /**
     * Adds a number column in the row header of the scrollpane, to match rows in the table. Assumes that table has already been added to a scollpane. If the table is not in a scrollpane nothing will happen.
     *
    ...
    public static void addNumberColumn(final JTable userTable, int startingNumber, boolean isRowSelectable) {
        ...
        if (parentContainer instanceof JViewport) {
            ...
            if (parentParentContainer instanceof JScrollPane) {
                ...
                final JTable rowHeadersTable = new JTable(new RowHeaderUtils().new RowHeadersTableModel(userTable.getModel()
...
//				if (isRowSelectable) {
                ...
                rowHeadersTable.getColumnModel().getColumn(0).setCellRenderer(new TableCellRenderer() {
                    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
                        ...
                        JComponent returnComponent = (JComponent) table.getTableHeader().getDefaultRenderer().getTableCellRendererComponent(table, value, false, false, -1, column);
...
//				}
                ...
                scrollPane.addComponentListener(new ComponentAdapter() {
                    ...
                    /**
                     * Whenever the component is resized need to re-adjust the column width if necessary. This method is also called when the screen is first layed out.
                     */
                ...
                new RowHeaderUtils().new TableListener(rowHeadersTable, userTable);
            }
        }
    }
    ...
    class RowHeadersTableModel extends AbstractTableModel {
        ...
        public void addNumber() {
            if (numbersList.size() != 0) {
                ...
                numbersList.add(numbersList.size(), startNumber);
            }
    ...
    /**
     * Adjusts the column width of the row headers table containg the number column. The font metrics are extracted from the label of the row at the bottom of the viewport and used to determing the appropriate width.
     * <p>
     * The reason why this method is important, is that when the row number increases by an extra digit the column needs to get wider. It also needs to shrink when scrolling to smaller digit numbers.
     *
    ...
    private static void adjustColumnWidth(final JTable rowHeadersTable, final JLabel label, int scrollBarValue) {
        ...
        if (fontMetrics != null && label.getText() != null) {
            ...
            rowHeadersTable.setPreferredScrollableViewportSize(new Dimension(widthFactor + 12, 100)); // height
            // is
            // ignored
    ...
    /**
     * Listener that joins the two tables; the main table, and the single column row number table. When either are moved or selected, then the affect is passed on to the other table.
     *
     * @author Oliver Watkins
     */
